# Vendored OpenFront core (headless simulation) - aeb8d60 pin

**This is one of two vendored engine trees**, kept side by side with
`src/vendor/openfront-core` (currently pinned to `dcc18d5...`) because no
single commit replays every real game correctly - see
`src/vendor/openfront-core/README.md`'s "Multiple vendored commits" section
for why, and `src/lib/replaySimCore.ts` (`KNOWN_ENGINE_COMMITS`,
`loadCreateGameRunner`) for the registry that picks between them per game
based on that game's own `gitCommit`. Everything below this point is the
original write-up from when this was the *only* vendored tree - still
accurate for how and why this specific commit was vendored, just no longer
describing the whole picture on its own.
