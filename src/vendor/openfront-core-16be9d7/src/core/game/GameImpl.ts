...
// Source: https://github.com/openfrontio/OpenFrontIO/blob/16be9d7c15d7abc115691def3a0b2aa559664705/src/core/game/GameImpl.ts
// Modified for this vendor build - see src/vendor/openfront-core-16be9d7/README.md
// for exactly what changed and why (dropped the `Unit | UnitView` union arm and the now-unused `client/view` import; repointed the `renderNumber` import at the local, non-upstream RenderNumber.ts extract instead of client/Utils.ts).
...
export class GameImpl implements Game {
  ...
  anyUnitNearby(
    ...
    return this.unitGrid.anyUnitNearby(
      ...
      searchRange,
      types,
      predicate as (unit: Unit) => boolean,
      playerId,
      includeUnderConstruction,
    );
  }
