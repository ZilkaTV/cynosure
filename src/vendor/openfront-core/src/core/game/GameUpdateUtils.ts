...
// Source: https://github.com/openfrontio/OpenFrontIO/blob/dcc18d5231af6253b0e991bf04a4c764982fe262/src/core/game/GameUpdateUtils.ts
// Modified for this vendor build - see src/vendor/openfront-core/README.md
// for exactly what changed and why (dropped the dead `applyStateUpdate` function (its only caller, client/view/PlayerView.ts, is not part of a headless replay) and the `client/render/types` PlayerState import it needed).
...
// applyStateUpdate (client-side PlayerState merge, keyed on
// client/render/types' PlayerState) intentionally dropped - it has no caller
// in the headless replay's reachable closure (only client/view/PlayerView.ts
// used it) and dropping it removes the only reason this file needed a
// client/render/types import.
