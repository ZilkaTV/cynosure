  ...
  return (
    <Link to="/quests" className="panel flex flex-col items-center gap-2 px-5 py-4 text-center transition-colors hover:border-accent/50">
      <p className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-slate-400">
        <Emoji char={EMOJI.star} className="h-3.5 w-3.5" /> Quests
      </p>
      ...
      <p className="text-[11px] text-slate-600">{countdown ? `Daily quests reset in ${countdown}` : 'Daily quests reset now'}</p>
    </Link>
  )
}
