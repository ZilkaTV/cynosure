 ...
 * means `players` has to be the FULL initial roster, in the real spawn
 * order, or every ID after the gap comes out different from the real game -
 * silently, since nothing throws, it just replays as if against a different
 * seed.
 *
 * raw.players (OpenFront's own summary endpoint) is NOT that full roster: it
 * only lists players who ended up with a stats entry, which someone who
 * disconnected in the first second or two of a public FFA never gets -
 * confirmed directly on a real game (8Z6BYvK9): turn 0's mark_disconnected
 * intents named 80 distinct clientIDs, raw.players only 75, and all 75 were
 * a subset of the 80. Every one of those 5 "missing" IDs then showed up as
 * "player with clientID X not found" during replay - not because that
 * player did anything unusual, but because the whole random-ID sequence
 * after them had already drifted off the real game's.
 *
 * Turn 0's mark_disconnected intents are themselves generated by iterating
 * the game's own player list right after spawn, so their order should match
 * the real spawn order this function needs - a strictly stronger and more
 * complete source than the summary endpoint, which is only used here as a
 * lookup for username/clanTag on the ids it does have.
 */
function buildFullPlayerRoster(raw: RawGameInfo, turn0: Turn | undefined): RawPlayer[] {
  const byId = new Map(raw.players.map((p) => [p.clientID, p]))
  const rosterIds = (turn0?.intents ?? [])
    .filter((i) => (i as { type?: string }).type === 'mark_disconnected')
    .map((i) => (i as unknown as { clientID: string }).clientID)
  if (rosterIds.length === 0) return raw.players // no turn 0 (empty/degenerate game) - fall back to what we have
  return rosterIds.map((id) => byId.get(id) ?? { clientID: id, username: id, clanTag: null })
}
...
function buildGameStartInfo(raw: RawGameInfo, turn0: Turn | undefined): GameStartInfo {
  ...
  return {
    ...
    config,
    players: buildFullPlayerRoster(raw, turn0).map((p) => ({
      clientID: p.clientID,
      username: p.username,
      clanTag: p.clanTag,
      isLobbyCreator: true,
...
async function loadRunner(gameId: string) {
  ...
  const gameStart = buildGameStartInfo(raw, rawTurns.find((t) => t.turnNumber === 0))
